...
class GenerateService extends BaseAdminService
{
    ...
    {
        $field = 'id,table_name,table_content,class_name,module_name,edit_type,order_type,parent_menu,relations,synchronous_number';
    ...
    public function checkFile($checkFile)
    {
        ...
        $controllerFile = $dir.'\\backend\\app\\adminapi\\controller\\'.$info['module_name'].'\\'.$info['class_name'].'.php';
        $modelFile = $dir.'\\backend\\app\\model\\'.$info['module_name'].'\\'.$info['class_name'].'.php';
        $validateFile = $dir.'\\backend\\app\\validate\\'.$info['module_name'].'\\'.$info['class_name'].'.php';
        $webViewFile = $dir.'\\frontend\\admin\\src\\views'.'\\'.$info['module_name'];
    ...
    {
                //获取系统模型
                $modulePath = dirname(root_path()) . '/backend/app/model/';
