...
class MemberConfigService extends BaseAdminService
{
    ...
    public function getGrowthRuleConfig(){
        ...
        if (!empty($config)) {
            if(!empty($config['grant']['member_register']))
            {
                $config['grant']['member_register']['content'] = "会员注册奖励".$config['grant']['member_register']['growth']."成长值";
            }
        }
        return $config;
    }
    ...
    public function getPointRuleConfig(){
        ...
        if (!empty($config)) {
            if(!empty($config['grant']['member_register']))
            {
                $config['grant']['member_register']['content'] = "会员注册奖励积分".$config['grant']['member_register']['point'];
            }
        }
        return $config;
    }
