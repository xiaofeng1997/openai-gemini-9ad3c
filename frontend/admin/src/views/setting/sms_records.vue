...
import { t } from '@/lang'
import { getSmsLog, getSmsNoticeList } from '@/api/sms'
import RecordsInfo from '@/views/setting/components/sms-records-info.vue'
    ...
    getSmsNoticeList().then(res => {
        ...
        res.data.forEach(item => {
            if (item.receiver_type == 1) {
                templateList.buyer.list.push({ name: item.name, value: item.key })
            } else {
                templateList.seller.list.push({ name: item.name, value: item.key })
            }
        })
    }).catch((e) => {
    })
