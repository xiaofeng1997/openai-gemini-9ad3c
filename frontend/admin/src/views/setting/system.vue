<template>
    <div class="main-container">
        ...
        <el-form class="page-form loading-box" :model="formData" label-width="150px" ref="formRef" :rules="formRules" v-loading="loading">
            ...
            <el-card class="box-card mt-[15px] !border-none" shadow="never">
                <h3 class="panel-title !text-[14px] bg-[#F4F5F7] p-3 border-[#E6E6E6] border-solid border-b-[1px]">{{ t('adminLoginSetting') }}</h3>
...
import { t } from '@/lang'
import { setWebsite, getWebsite, getService, getConfigLogin } from '@/api/sys'
...
const setFormData = async () => {
    ...
    const login_data = await (await getConfigLogin()).data
    formData['is_captcha'] = login_data.is_captcha
    formData['bg'] = login_data.bg
...
const save = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate(async (valid) => {
        if (valid) {
            ...
            // 后端 setWebSite 方法需要 is_captcha 和 login_bg_img 字段
            const websiteData = {
                ...formData,
                login_bg_img: formData.bg
            }

            try {
                await setWebsite(websiteData)
            } catch (e) {
            } finally {
                loading.value = false
            }
