                    ...
                    <el-form :model="formData" label-width="70px" class="page-form">
                        ...
                        <el-form-item :label="t('className')">
                            <div>
                                ...
                                <p class="text-[12px] text-[#a9a9a9] leading-normal mt-[5px]">生成代码所属文件名称，controller，model，service，等类型文件等</p>
                    ...
                    <el-form :model="formData" class="page-form" label-width="140px" ref="formRef" :rules="rules">
                        ...
                        <el-form-item :label="t('orderColumnName')">
                            <div>
                                ...
                                <p class="text-[12px] text-[#a9a9a9] leading-normal mt-[5px]">列表排序规则字段名</p>
                            </div>
                        ...
                        <el-form-item :label="t('orderType')" v-if="formData.order_column_name">
                            <div>
                                ...
                                <p class="text-[12px] text-[#a9a9a9] leading-normal mt-[5px]">列表排序规则类型</p>
                            </div>
                        ...
                        <el-form-item :label="t('menuType')">
                            ...
                            <el-tree-select class="input-width" clearable
                                              v-model="formData.parent_menu" :props="{ label: 'menu_name', value: 'menu_key' }"
                                  :data="sysMenuList" check-strictly
...
import editAssociated from '@/views/tools/code/components/edit-associated.vue'
import editViewType from '@/views/tools/code/components/edit-view-type.vue'
import editVerify from '@/views/tools/code/components/edit-verify.vue'
import { getGenerateTableInfo, editGenerateTable, generatorCheckFile, generateCreate } from '@/api/tools'
import { getSystemMenu } from '@/api/sys'
...
const controller = computed(() => {
    return `app\\adminapi\\controller${formData.module_name ? '\\' + formData.module_name : ''}${formData.class_name ? '\\' + formData.class_name : ''}`
})
const dataModel = computed(() => {
    return `app\\model${formData.module_name ? '\\' + formData.module_name : ''}${formData.class_name ? '\\' + formData.class_name : ''}`
})
const validator = computed(() => {
    return `app\\validate${formData.module_name ? '\\' + formData.module_name : ''}${formData.class_name ? '\\' + formData.class_name : ''}`
})
const webView = computed(() => {
    return 'admin\\src'
})
const routerView = computed(() => {
    return `app\\adminapi\\route${formData.module_name ? '\\' + formData.module_name : ''}`
})
