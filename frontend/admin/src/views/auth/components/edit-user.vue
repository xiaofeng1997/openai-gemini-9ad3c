<template>
    <el-dialog v-model="showDialog" :title="popTitle" width="500px" :destroy-on-close="true">
        <el-form :model="formData" label-width="90px" ref="formRef" :rules="formRules" class="page-form" v-loading="loading">
                ...
                <el-select :model-value="uid" :placeholder="t('accountNumberPlaceholder')" class="input-width" filterable clearable :allow-create="true" @change="selectUser" :default-first-option="true">
                    <el-option v-for="item in userList" :key="item.uid" :label="item.username" :value="item.uid">
                        <div class="flex items-center">
                            ...
                            <img src="@/assets/images/member_head.png" alt="" class="mr-[10px] w-[24px]" v-else>
                            {{ item.username }}
                        </div>
                    </el-option>
                </el-select>
            ...
            <el-form-item :label="t('positionName')" prop="position_id">
                <el-select v-model="formData.position_id" :placeholder="t('positionNamePlaceholder')" clearable class="input-width">
...
import { getAllUserList,getUserInfo, addUser, editUser } from '@/api/user'
import { allRole, getDeptTree, getPositionAll } from '@/api/sys'
