...
import { getDiyBottomConfig, setDiyBottomConfig } from '@/api/diy_config'
...
const getDiyBottomFn = () => {
    ...
    getDiyBottomConfig({
        ...
        loading.value = false
        diyBottomData.key = res.data.key
        diyBottomData.value = res.data.value
    }).catch(() => {
        loading.value = false
    })
}
