<template>
    <view class="w-screen h-screen bg-[var(--page-bg-color)]" :style="themeColor()">
        <mescroll-body ref="mescrollRef" @init="mescrollInit" :down="{ use: false }" @up="getCashOutAccountListFn">
            ...
            <view class="card-template sidebar-margin my-[var(--top-m)] flex items-center" @click="redirect({ url: '/pages/member/account_edit', param: { type: accountType, mode } })">
...
import { getCashOutAccountList, deleteCashoutAccount } from '@/api/member'
...
const editAccount = (item: any) => {
    const url = `/pages/member/account_edit`
