    ...
    private handleAuthError(code: number) {
        switch (code) {
            case 401:
                ...
                break;
            case 402:
                if (currRoute().indexOf('pages/index/close') != -1) return;
                redirect({ url: '/pages/index/close', mode: 'reLaunch' })
                break;
            case 403:
                if (currRoute().indexOf('pages/index/nosite') != -1) return;
                redirect({ url: '/pages/index/nosite', mode: 'switchTab' })
                break;
        }
    }
