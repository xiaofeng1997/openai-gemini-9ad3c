...
onLaunch((data: any) => {
    ...
    useSystemStore().getInitFn(async () => {
        ...
        // 设置主色调，确保在theme_color存储之后调用
        setTimeout(() => {
            setThemeColor(url)
        }, 100)

        // 判断账号锁定后在登录注册页面不进行请求三方登录注册,防止在页面出不去 member_lock 为账号锁定标识
        if (uni.getStorageSync('member_lock') && (['pages/auth/index', 'pages/auth/login', 'pages/auth/register', 'pages/auth/resetpwd'].indexOf(url) != -1) && (loginConfig.is_username || loginConfig.is_mobile || loginConfig.is_bind_mobile)) {
            return false
        }
